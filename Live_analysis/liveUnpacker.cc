...
#include "TTreeUnpackerPC.h"
#include "RNTupleUnpackerPC.h"
#include <getopt.h>
#include <libgen.h>
#include <deque>
...
void usage() {
  printf("Usage: liveUnpacker.exe [ options ] <kind> /path/to/input /path/to/outputs \n");
...
std::unique_ptr<UnpackerBase> makeUnpacker(const std::string &obj,
  ...
  if(kind == "ttree"){
    unpacker = std::make_unique<TTreeUnpackerPC>();
  } else if(kind == "rntuple"){
    unpacker = std::make_unique<RNTupleUnpackerPC>();
  } else {
...
int main(int argc, char **argv) {
  ...
  std::unique_ptr<UnpackerBase> unpacker = makeUnpacker(obj, kind, compressionLevel);
  if (!unpacker)
    return 2;
  if (threads == -1) {
