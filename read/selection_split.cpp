...
void saveplot(float hpt, float ipt, float lpt, bool el, bool m, bool iso, std::string PiIso="All", bool save_event=true){


    TStopwatch timer0;
    timer0.Start();
    ROOT::RDataFrame df("Events","stream/stream2.root");
    // auto df_event = df.Define("Triplet_idxs", TripletMaker(60.,100.,hpt,ipt,lpt, el, m, iso, PiIso, 0.01, 0.25, 0.45), {"pid", "pt", "eta", "phi", "mass", "charge"});
    // ROOT::RDataFrame df("Events","capture_data/l1Nano_WTo3Pion_PU200_iso.root");
    auto df1 = df.Define("Triplet_idxs", TripletMaker(60.,100.,hpt,ipt,lpt, el, m, iso, PiIso, 0.01, 0.25, 0.45), {"pid", "pt", "eta", "phi", "mass", "charge"}).Filter(notempty, {"Triplet_idxs"}).Define("Triplet_invmass", triplet_mass, {"Triplet_idxs","pt", "eta", "phi", "mass"});
    timer0.Stop();
    double tcpu0 = timer0.CpuTime(), treal0 = timer0.RealTime();
    std::cout << "timer(sel.): " << tcpu0 << ", real: " << treal0 << std::endl;
    
    TStopwatch timer1;
    timer1.Start();

    if(save_event){
        // auto df1 = df_event.Filter(notempty, {"Triplet_idxs"}).Define("Triplet_invmass", triplet_mass, {"Triplet_idxs", "pt", "eta", "phi", "mass"});
    
        //SNAPSHOT
        df1.Snapshot("SelectedEvents", "snapshots/snapshot_events.root", {"pt", "eta", "phi"});
        timer1.Stop();
        double tcpu1 = timer1.CpuTime(), treal1 = timer1.RealTime();
        std::cout << "timer(save event): " << tcpu1 << ", real: " << treal1 << std::endl;
    }
    else{ // select only the features of the pions
        auto filtered_df = df1.Define("Pions_pt",  Return_triplet, {"pt", "Triplet_idxs"})
                              .Define("Pions_eta", Return_triplet, {"eta","Triplet_idxs"})
                              .Define("Pions_phi", Return_triplet, {"phi","Triplet_idxs"});

        //SNAPSHOT
        filtered_df.Snapshot("SelectedTriplet", "snapshots/snapshot_triplet.root", {"Pions_pt","Pions_eta","Pions_phi"});
        timer1.Stop();
        double tcpu1 = timer1.CpuTime(), treal1 = timer1.RealTime();
        std::cout << "timer(save triplet): " << tcpu1 << ", real: " << treal1 << std::endl;
    }

}
// Poi c'è sempre da spezzare i filtri
// fai le slide con i risultati
...
int main(){
    ...
    TStopwatch timer;
    timer.Start();
    // void saveplot(float hpt, float ipt, float lpt, bool el, bool m, bool iso, std::string PiIso="All", bool save_event){
    saveplot(18,15,12, true,true,true, "All", false);
    timer.Stop();
    double tcpu = timer.CpuTime(), treal = timer.RealTime();
    std::cout << "timer(tot): " << tcpu << ", real: " << treal << std::endl;
    return 0;
}